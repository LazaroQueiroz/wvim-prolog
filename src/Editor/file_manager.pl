...
save_file(State, _, Args, NewState) :-
    State = editor_state(_, _, _, _, _, Filename, _, _, _, _, _, _, _),
...
write_file(State, Path, NewState) :-
    State = editor_state(_, PT, Cursor, View, _, FN, SB, _, U, R, VS, CopyText, Search),
...
save_and_quit(State, Force, Args, FinalState) :-
    ...
    TempState = editor_state(_, _, _, _, _, _, SB, _, _, _, _, _, _),
...
set_error(editor_state(_, PT, Cursor, View, FS, FN, _, _, U, R, VS, CopyText, Search), Msg, editor_state(normal, PT, Cursor, View, FS, FN, status_bar(exception,Msg), "", U, R, VS, CopyText, Search)).
